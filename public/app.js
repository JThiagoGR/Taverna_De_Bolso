...
canvas.addEventListener('mouseup',e=>{if(wallStart){const[x,y]=getPos(e);const end=[Math.round(x/50)*50,Math.round(y/50)*50];if(wallStart[0]!==end[0]||wallStart[1]!==end[1])socket.emit('addWall',{room:me.room,wall:{x1:wallStart[0],y1:wallStart[1],x2:end[0],y2:end[1]}});wallStart=null;}rulerStart=null;dragging=null;});
...
canvas.addEventListener('touchend',e=>{e.preventDefault();const t=e.changedTouches[0];const n=Date.now();const[x,y]=getPos(t);if(n-lastTap<300&&Math.abs(x-lastX)<30&&Math.abs(y-lastY)<30){let c=null;players.forEach(p=>{if(Math.hypot(p.x-x,p.y-y)<25)c=p;});if(c&&((me.pid&&c.ownerId===me.pid)||me.isMaster)&&!c.isNpc){openPlayerSheet(c.id);}}lastTap=n;lastX=x;lastY=y;if(wallStart){const end=[Math.round(x/50)*50,Math.round(y/50)*50];if(wallStart[0]!==end[0]||wallStart[1]!==end[1])socket.emit('addWall',{room:me.room,wall:{x1:wallStart[0],y1:wallStart[1],x2:end[0],y2:end[1]}});wallStart=null;}rulerStart=null;dragging=null;});
