...
socket.on('fogUpdated',f=>{fogEnabled=f;draw();});
socket.on('lightSet',l=>{globalLight=l;draw();});
socket.on('lightUpdated',l=>{globalLight=l;draw();});
...
canvas.addEventListener('mousemove',e=>{if(tool==='pan'&&dragging==='pan'){offsetX+=e.clientX-canvas.dataset.px;offsetY+=e.clientY-canvas.dataset.py;canvas.dataset.px=e.clientX;canvas.dataset.py=e.clientY;if(me&&me.isMaster)socket.emit('setZoom',{room:me.room,zoom:scale,offsetX,offsetY});draw();}else if(dragging&&dragging!=='pan'){if(!me.isMaster&&dragging.isNpc){dragging=null;return;}const[x,y]=getPos(e);dragging.x=x;dragging.y=y;socket.emit('move',{room:me.room,id:dragging.id,x,y});draw();}else if(wallStart){const[x,y]=getPos(e);draw();ctx.save();ctx.translate(offsetX,offsetY);ctx.scale(scale,scale);ctx.strokeStyle='#c97c3d';ctx.lineWidth=2/scale;ctx.beginPath();ctx.moveTo(wallStart[0],wallStart[1]);ctx.lineTo(Math.round(x/50)*50,Math.round(y/50)*50);ctx.stroke();ctx.restore();}else if(rulerStart){rulerEnd=getPos(e);window.sharedRuler={a:rulerStart,b:rulerEnd};socket.emit('setRuler',{room:me.room,ruler:window.sharedRuler});draw();}});
...
canvas.addEventListener('touchmove',e=>{e.preventDefault();if(e.touches.length===1){const t=e.touches[0];if(tool==='pan'&&dragging==='pan'){offsetX+=t.clientX-canvas.dataset.px;offsetY+=t.clientY-canvas.dataset.py;canvas.dataset.px=t.clientX;canvas.dataset.py=t.clientY;if(me&&me.isMaster)socket.emit('setZoom',{room:me.room,zoom:scale,offsetX,offsetY});draw();}else if(dragging&&dragging!=='pan'){if(!me.isMaster&&dragging.isNpc){dragging=null;return;}const[x,y]=getPos(t);dragging.x=x;dragging.y=y;socket.emit('move',{room:me.room,id:dragging.id,x,y});draw();}else if(wallStart){const[x,y]=getPos(t);draw();ctx.save();ctx.translate(offsetX,offsetY);ctx.scale(scale,scale);ctx.strokeStyle='#c97c3d';ctx.lineWidth=2/scale;ctx.beginPath();ctx.moveTo(wallStart[0],wallStart[1]);ctx.lineTo(Math.round(x/50)*50,Math.round(y/50)*50);ctx.stroke();ctx.restore();}else if(rulerStart){rulerEnd=getPos(t);window.sharedRuler={a:rulerStart,b:rulerEnd};socket.emit('setRuler',{room:me.room,ruler:window.sharedRuler});draw();}}});
